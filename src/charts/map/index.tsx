...
export const Map: React.FC<MapProps> = ({ mapData, mapAttr, dataToMap, connection, classes, color }) => {
  const width =1000
  const height = 1000
  ...
  const initScale =150
  const initialProjection = geoMercator().scale(initScale).center(initCenter)
  ...
  const offset: [number,number]  = [width - (bounds[0][0] + bounds[1][0])/2,
  ...
  const colors = [ "rgba(135, 24, 157, 0.2)",
  "rgba(135, 24, 157, 0.8)",
  "rgba(135, 24, 157, 0.6)",
  "rgba(135, 24, 157, 0.4)",
  "rgba(135, 24, 157, 0.2)",
  ...
  const projection = geoMercator().scale(scale).center(initCenter).translate(offset)
  const pathGenerator = geoPath().projection(projection)

  return (
    <div style={{width: "90%", height: "100%", margin:"auto"}}>

    <svg  width = {"100%"} height={"100%"} viewBox={`0 0 ${width} ${height }`} style={{ backgroundColor:"none" }}>
